...
class GRUCell(tf.contrib.rnn.RNNCell):
    ...
    def __call__(self, inputs, state, scope=None):
        ...
        with tf.variable_scope(scope):
            ...
            U_r = tf.get_variable('U_r', shape = [self.input_size, self.state_size], dtype = tf.float32, initializer = xavier_weight_init())
            W_r = tf.get_variable('W_r', shape = [self.state_size, self.state_size], dtype = tf.float32, initializer = xavier_weight_init())
            b_r = tf.get_variable('b_r', shape = [self.state_size,], dtype = tf.float32, initializer = tf.constant_initializer(0.0))
            
            U_z = tf.get_variable('U_z', shape = [self.input_size, self.state_size], dtype = tf.float32, initializer = xavier_weight_init())
            W_z = tf.get_variable('W_z', shape = [self.state_size, self.state_size], dtype = tf.float32, initializer = xavier_weight_init())
            b_z = tf.get_variable('b_z', shape = [self.state_size,], dtype = tf.float32, initializer = tf.constant_initializer(0.0))
            
            U_o = tf.get_variable('U_o', shape = [self.input_size, self.state_size], dtype = tf.float32, initializer = xavier_weight_init())
            W_o = tf.get_variable('W_o', shape = [self.state_size, self.state_size], dtype = tf.float32, initializer = xavier_weight_init())
            b_o = tf.get_variable('b_o', shape = [self.state_size,], dtype = tf.float32, initializer = tf.constant_initializer(0.0))
            #z_t = sigmoid(x_t U_z + h_{t-1} W_z + b_z)
            #r_t = sigmoid(x_t U_r + h_{t-1} W_r + b_r)
            #o_t = tanh(x_t U_o + r_t * h_{t-1} W_o + b_o)
            #h_t = z_t * h_{t-1} + (1 - z_t) * o_t
            ...
            #print('a.shape', tf.matmul(inputs, U_z).shape)
            #print('b.shape', tf.matmul(state, W_z).shape)
            #print('bz.shape', b_z.shape)
