...
def make_windowed_data(data, start, end, window_size = 1):
    ...
    ### YOUR CODE HERE (5-20 lines)
        #insert start and end in the sentense
        ...
        a = np.arange(len(new_sentence)-2*window_size)
        a = a.reshape(len(a), -1)
        b = np.repeat(a, 2*window_size+1, axis=1) + np.arange(2*window_size+1)
        
        windows = np.array(new_sentence)[b]
        for i in range(windows.shape[0]):
            windowed_data.append((windows[i].flatten().tolist(), labels[i]))
...
class WindowModel(NERModel):
    ...
    def add_placeholders(self):
        ...
        self.input_placeholder = tf.placeholder(tf.int32, [None, self.config.n_window_features], name='input_placeholder')
        self.labels_placeholder = tf.placeholder(tf.int32, [None, ], name = 'labels_placeholder')
        self.dropout_placeholder = tf.placeholder(tf.int32, None, name='dropout_placeholder')
        ### END YOUR CODE
    ...
    def add_prediction_op(self):
        ...
        self.W = tf.Variable(weight_initializer((self.config.n_window_features*self.config.embed_size, self.config.hidden_size)))
    ...
    def train_on_batch(self, sess, inputs_batch, labels_batch):
        ...
        #print('label_batch first 10:', labels_batch[:10])
